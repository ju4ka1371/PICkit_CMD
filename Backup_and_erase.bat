...
title PIC MCU Backup (+full erase) >nul

:: =========================================================================================================
::  pk2_backup_and_erase.bat
...
::  3. Erases the WHOLE chip (Program Memory + Config bits + EEPROM)
::
...
if not "%BACKUP_ERR%"=="0" (
    echo.
    echo [ERROR] Backup failed. Erase CANCELLED.
	echo.
    pause
    exit /b 1
)
...
if not exist "%BACKUPFILE%" (
    echo.
    echo [ERROR] Backup file was not created. Erase CANCELLED.
	echo.
    pause
    exit /b 1
)
...
if "%BSIZE%"=="0" (
    echo.
    echo [ERROR] Backup file is empty. Erase CANCELLED.
	echo.
    pause
    exit /b 1
)
...
echo.
choice /M "Erase the WHOLE chip (Program Memory, Config bits and EEPROM)"
if errorlevel 2 (
...
:: --- Step 3: erase the whole chip (Bulk Erase) ---
echo.
echo ================================================================================
echo   Step 3: erasing the whole chip
echo ================================================================================
set "ERASELOG=%TEMP%\pk2_erase.log"
"%PK2CMD%" /P /E > "%ERASELOG%" 2>&1
...
if not "%ERASE_ERR%"=="0" (
    echo.
    echo [ERROR] Erase failed ^(code %ERASE_ERR%^).
...
echo.
echo Done! The chip has been fully erased.
